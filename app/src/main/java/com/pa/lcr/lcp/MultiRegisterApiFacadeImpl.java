 ...
 *
 * ✅ Fix (A4-Transport wiring):
 * - api_mediaCheck("usb") doit refléter l'état du TransportIo "USB" (MediaTransportManager),
 *   sinon /v1/lcp/connect peut échouer alors que /v1/media/check dit OK.
 * - api_openPingUsb publie le port USB dans MediaTransportManager (best-effort via réflexion)
 *   afin que RegisterSessionManager (TransportIo strict) puisse créer une session LCP.
 * - implémente explicitement les overloads media-aware (api_connectLcp(...,media,bt_mac), etc.)
 *   pour éviter le fallback default d'ApiFacade vers le chemin legacy.
 */
public final class MultiRegisterApiFacadeImpl implements ApiFacade {
    ...
    public ApiResult api_mediaCheck(String media, String bt_mac) {
        try {
            ...
            if ("usb".equals(m)) {
                ...
                // ✅ Source de vérité: TransportIo publié (MediaTransportManager)
                TransportIo tio = (mediaMgr != null) ? mediaMgr.getByKey("USB") : null;
                boolean ok = (tio != null && tio.isOpen());
                d.put("connected", ok ? 1 : 0);

                // Diagnostic additionnel: UsbSession port (legacy UI)
                UsbSerialPort p = null;
                try { p = UsbSession.getPort(); } catch (Exception ignored) {}
                d.put("usbSessionPort", (p != null) ? 1 : 0);

                if (ok) return ApiResult.ok("MediaCheck: 1 - USB connecté", d);

                // Si port legacy présent mais TransportIo absent -> hint
                if (p != null) {
                    d.put("note", "UsbSession port ouvert mais TransportIo USB non publié");
                }
            ...
            if ("bt".equals(m) || "bluetooth".equals(m)) {
                ...
                TransportIo tio = (mediaMgr != null) ? mediaMgr.getByKey(key) : null;
                boolean ok = (tio != null && tio.isOpen());
                d.put("connected", ok ? 1 : 0);
    ...
    /**
     * Open/Ping USB:
     * - ouvre un port série et le place dans UsbSession (legacy)
     * - ✅ publie aussi dans MediaTransportManager (TransportIo USB) via réflexion (best-effort)
    ...
    public ApiResult api_openPingUsb() {
        try {
            ...
            if (existing != null) {
                publishUsbToMediaMgrBestEffort(null, existing, "USB prêt (UsbSession)");
                JSONObject d = new JSONObject();
                d.put("usb_ready", 1);
            ...
            try {
                ...
                // ✅ publish to MediaTransportManager so TransportIo path is ready
                publishUsbToMediaMgrBestEffort(dev, port, "USB prêt (API open-ping)");
    ...
    private void publishUsbToMediaMgrBestEffort(UsbDevice dev, UsbSerialPort port, String reason) {
        if (mediaMgr == null || port == null) return;
        try {
            // Try method signature: onUsbReady(UsbDevice, UsbSerialPort, String)
            Method m = mediaMgr.getClass().getMethod("onUsbReady", UsbDevice.class, UsbSerialPort.class, String.class);
            m.invoke(mediaMgr, dev, port, reason);
            return;
        } catch (Throwable ignored) {}
        try {
            // Try alternate signature: onUsbReady(UsbSerialPort)
            Method m2 = mediaMgr.getClass().getMethod("onUsbReady", UsbSerialPort.class);
            m2.invoke(mediaMgr, port);
        } catch (Throwable ignored) {}
    }

    // =========================================================
    // Helpers (node/from + session)
    // =========================================================
    ...
    private DeliveryController requireSessionUsb(Integer nodeDec, Integer fromDec) {
        UsbSerialPort port = null;
        try { port = UsbSession.getPort(); } catch (Exception ignored) {}
        if (port == null) return null;
        int n = normNode(nodeDec);
        int f = normFrom(fromDec);
        notifyNodeSeen(n, f);
        // legacy overload: RegisterSessionManager handles UsbSerialPort -> TransportIo
        return sessions.getOrCreate(n, f, port);
    }

    private DeliveryController requireSessionBt(Integer nodeDec, Integer fromDec, String btMac) {
        if (mediaMgr == null) return null;
        if (btMac == null || btMac.trim().isEmpty()) return null;
        int n = normNode(nodeDec);
        int f = normFrom(fromDec);
        notifyNodeSeen(n, f);
        String key = MediaTransportManager.btKey(btMac.trim());
        TransportIo tio = mediaMgr.getByKey(key);
        if (tio == null || !tio.isOpen()) return null;
        return sessions.getOrCreate(key, n, f, tio);
    }

    private DeliveryController requireSession(Integer nodeDec, Integer fromDec, String media, String btMac) {
        String m = (media == null) ? "usb" : media.trim().toLowerCase(Locale.ROOT);
        if (m.isEmpty()) m = "usb";
        if ("usb".equals(m)) return requireSessionUsb(nodeDec, fromDec);
        if ("bt".equals(m) || "bluetooth".equals(m)) return requireSessionBt(nodeDec, fromDec, btMac);
        return null;
    }
    ...
    private ApiResult failTransportLevel(String media, String btMac, String where) {
        ...
        if ("usb".equals(m)) {
            return ApiResult.fail("Connect LCP: 0 - USB non prêt.", "ERR_USB_PORT_NOT_READY", d);
        }
        if ("bt".equals(m) || "bluetooth".equals(m)) {
    ...
    // =========================================================
    // Legacy wrappers (ApiFacade abstract)
    // =========================================================
    ...
    @Override
    public ApiResult api_registerValidate(String numero_livraison, Integer expected_lcrnode_dec, String expected_serial_id,
                                         Integer expected_product_number, String expected_compartment) {
    ...
    @Override
    public ApiResult api_deliveryOneShotStart(Integer lcrnode_dec, Integer from_dec, String numero_livraison,
                                             int product1to16, double presetNetL, String compartment) {
    ...
    // =========================================================
    // ✅ Media-aware overrides (fix default fallback)
    // =========================================================
    @Override
    public ApiResult api_connectLcp(Integer lcrnode_dec, Integer from_dec, String media, String bt_mac) {
        ...
        if (dc == null) return failTransportLevel(media, bt_mac, "api_connectLcp");
    ...
    public ApiResult api_deliveryAlignA(Integer lcrnode_dec, Integer from_dec, String media, String bt_mac) {
        DeliveryController dc = requireSession(lcrnode_dec, from_dec, media, bt_mac);
        if (dc == null) return failTransportLevel(media, bt_mac, "api_deliveryAlignA");
    ...
    public ApiResult api_deliveryStartC(Integer lcrnode_dec, Integer from_dec, int product1to16, double presetNet, String media, String bt_mac) {
        ...
        DeliveryController dc = requireSession(node, from, media, bt_mac);
        if (dc == null) return failTransportLevel(media, bt_mac, "api_deliveryStartC");
    ...
    public ApiResult api_deliveryOneShotStart(Integer lcrnode_dec, Integer from_dec, String numero_livraison,
        ...
        int from = normFrom(from_dec);
        DeliveryController dc = requireSession(node, from, media, bt_mac);
        if (dc == null) return failTransportLevel(media, bt_mac, "api_deliveryOneShotStart");
    ...
    public ApiResult api_registerValidate(String numero_livraison, Integer expected_lcrnode_dec, Integer from_dec,
        ...
        if (dc == null) return failTransportLevel(media, bt_mac, "api_registerValidate");
    ...
    public ApiResult api_ticketReprintCurrent(Integer lcrnode_dec, Integer from_dec, String media, String bt_mac) {
        DeliveryController dc = requireSession(lcrnode_dec, from_dec, media, bt_mac);
        if (dc == null) return failTransportLevel(media, bt_mac, "api_ticketReprintCurrent");
        return dc.api_ticketReprintCurrent();
    }

    // DB dump / tick wait (legacy)
    @Override
    ...
    public ApiResult api_tickWait(Integer lcrnode_dec, Long since_seq, Integer wait_ms) {
        ...
        DeliveryController dc = requireSessionUsb(node, from);
        if (dc == null) {
            JSONObject d = new JSONObject();
            try { d.put("level", "MEDIA"); d.put("where", "api_tickWait"); d.put("media", "usb"); } catch (Exception ignored) {}
            return ApiResult.fail("Tick: 0 - USB non prêt.", "ERR_USB_PORT_NOT_READY", d);
        }
