...
safeJsonPut(data, "state_job", "DONE");
            ...
            if (job.lastOkData != null) {
                ...
                try {
                    ...
                    // ✅ FIX: corriger fs_net_l / fs_gross_l avec les valeurs du tick.
                    // Quand READ_FAIL, gross_total_end n'est pas à jour → delta faux.
                    // Le tick contient les dernières valeurs lues directement sur le registre.
                    // Comportement uniforme LCR-II (Field #44/#45 directs) et LC3 (calcul delta).
                    double tickNet   = tick.optDouble("net",   -1);
                    double tickGross = tick.optDouble("gross", -1);
                    if (tickNet >= 0 && tickGross >= 0) {
                        JSONObject result = data.optJSONObject("result");
                        if (result != null) {
                            safeJsonPut(result, "fs_net_l",   tickNet);
                            safeJsonPut(result, "fs_gross_l", tickGross);
                            safeJsonPut(data,   "net",        tickNet);
                            safeJsonPut(data,   "gross",      tickGross);
                        }
                    }
                } catch (Exception ignored) {}
