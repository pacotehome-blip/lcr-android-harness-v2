 ...
 safeJsonPut(data, "media", (job.media == null ? resolveActiveMedia() : job.media));
    ...
    public ApiResult api_testImprimante() {
        ...
        try {
            ...
            // ✅ CORRIGÉ (14 août 2026, demande Paul — "dans le support on
            // n'a pas la trace du test") — emitLog() ci-dessus envoie
            // seulement au panneau de log LOCAL du tab (listener.onLog),
            // PAS à Support. LogBus est le vrai système qui alimente
            // Support (confirmé plus tôt aujourd'hui pour les événements
            // média) — manquant ici, corrigé.
            try {
                int nodeForLog = (link != null) ? (link.getToAddr() & 0xFF) : 0;
                com.pa.lcr.lcp.log.LogBus.ui(nodeForLog, "[TEST-IMPRIMANTE] " + verdict
                        + " (prnStatus=0x" + String.format("%02X", ms.prnStatus) + ")");
            } catch (Exception ignored) {}
            ...
            try {
                int nodeForLog = (link != null) ? (link.getToAddr() & 0xFF) : 0;
                com.pa.lcr.lcp.log.LogBus.err(nodeForLog, "DeliveryController.api_testImprimante", e);
            } catch (Exception ignored) {}
