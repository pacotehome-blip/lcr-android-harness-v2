 ...
 *
 * FIX CRASH:
 * - ✅ Throttle/coalesce refresh log (ne pas rebuild/setText à chaque event).
 * - ✅ Réduit le nombre de lignes affichées.
 */
public class RegisterTabFragment extends Fragment {
  ...
  // ---------------------------
  private static final int TAB_LOG_MAX_LINES = 600;     // <-- au lieu de 1200
  private static final long LOG_REFRESH_MIN_MS = 300;   // <-- max ~3 fps
  ...
  private final DeliveryControllerPort.Listener uiListener = new DeliveryControllerPort.Listener() {
    ...
    public void onStateChanged(DeliveryState state) {
      ui.post(() -> {
        ...
        // ✅ FIX: ne pas spammer refreshLogView() ici
        scheduleLogRefresh();
      });
    }
  ...
  private void wireUi() {
    ...
    cbLogTs.setOnCheckedChangeListener((b, checked) -> {
      ...
      // garder: ça réduit/augmente les logs côté controller (utile)
    ...
    cbTxRx.setOnCheckedChangeListener((b, checked) -> {
      // garder: ça coupe/active la production IO dans controller
  ...
  private void refreshLogView() {
    ...
    String text = LogBus.buildText(LogBus.filterNodeUIIOAPI(node, includeIo, logViewSinceMs), TAB_LOG_MAX_LINES);
    txtLog.setText(text);
  }
