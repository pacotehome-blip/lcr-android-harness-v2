...
public class MainActivity extends AppCompatActivity {
 ...
 // liveTick running flag (évite double scheduling)
 ...
 // ✅ LIVE tick: actif uniquement en RUNNING_FLOWING (FLOW_ACTIVE=ON)
 private final Runnable liveTick = new Runnable() {
  ...
  public void run() {
   ...
   if (controller.getState() != DeliveryState.RUNNING_FLOWING) {
    // stop silently when not flowing
 ...
 // Démarre le liveTick (uniquement si pas déjà actif)
 ...
 private void connectLcp() {
  ...
  controller.setListener(new DeliveryControllerPort.Listener() {
   ...
   public void onStateChanged(DeliveryState state) {
    ui.post(() -> {
     ...
     // Gestion liveTick: actif uniquement en RUNNING_FLOWING
     if (state == DeliveryState.RUNNING_FLOWING) {
  ...
  // liveTick ne tourne QUE pendant RUNNING_FLOWING
  stopLiveTick();
  // Snapshot NET/GROSS (une seule lecture) pour affichage après connect/resync
  if (controller != null) controller.requestLiveSnapshot();
 }
